...
void init_mock_game() {
    ...
    for (int i = 0; i < MOCK_TEXT_MAX; i++) {
        ...
        else if (7 <= i && i <= 10) mock_text[i] = Object::create(path, mockQ1, 0, 50, false); //첫번째 문제 화면 텍스트들
        else if (11 <= i && i <= 14) mock_text[i] = Object::create(path, mockQ2, 0, 50, false); //두번째 문제 화면 텍스트들
